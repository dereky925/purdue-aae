...
%          solutions, including the costates and control inputs over time.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
vtheta0 = sqrt(mu / r0);  % tangential velocity (AU/yr)
...
costate_sol_all = zeros(N,4);  % [lambda_r(0), lambda_theta(0), lambda_vr(0), lambda_vtheta(0)]
...
% New cell arrays to store costates (columns 5-8)
all_lambda_r      = cell(numToPlot,1);
all_lambda_theta  = cell(numToPlot,1);
all_lambda_vr     = cell(numToPlot,1);
all_lambda_vtheta = cell(numToPlot,1);

% New cell arrays to store control inputs and their magnitude
all_u1    = cell(numToPlot,1);
all_u2    = cell(numToPlot,1);
all_uMag  = cell(numToPlot,1);
...
for k = 1:numToPlot
    ...
    % Extract costates (columns 5-8)
    lambda_r_sol      = Z_out(:,5);
    lambda_theta_sol  = Z_out(:,6);
    lambda_vr_sol     = Z_out(:,7);
    lambda_vtheta_sol = Z_out(:,8);
    
    % Compute control input: u1 = -0.5*lambda_vr, u2 = -0.5*lambda_vtheta
    u1 = -0.5 * lambda_vr_sol;
    u2 = -0.5 * lambda_vtheta_sol;
    u_mag = sqrt(u1.^2 + u2.^2);
    
    % Convert polar to Cartesian for state trajectory
    ...
    all_lambda_theta{k}  = lambda_theta_sol;
    all_lambda_vr{k}     = lambda_vr_sol;
    all_lambda_vtheta{k} = lambda_vtheta_sol;
    
    % Store control inputs and magnitude
    all_u1{k}   = u1;
    all_u2{k}   = u2;
    all_uMag{k} = u_mag;
end
...
%% Figure 4: Costate Evolution (already showing the evolution of lambda_r, lambda_\theta, lambda_{v_r}, lambda_{v_\theta})
figure('Color','white','Position',[1200 500 1000 1000]);
% lambda_r(t)
subplot(2,1,1); hold on; grid on;
title('\lambda_r vs. Time');
xlabel('Time (yr)'); ylabel('Position');
lineWidthStart = startValue;
for k = 1:numToPlot
    plot(all_tSol{k}, all_lambda_r{k}, [colorList{k},'-'], ...
         'LineWidth', lineWidthStart, 'DisplayName', labels{k});
    lineWidthStart = lineWidthStart - decrement;
end
lineWidthStart = startValue;
for k = 1:numToPlot
    plot(all_tSol{k}, all_lambda_theta{k}, [colorList{k},'-'], ...
         'LineWidth', lineWidthStart, 'DisplayName', labels{k});
    lineWidthStart = lineWidthStart - decrement;
end
legend('Location','best');
...
title('\lambda_{v_r} vs. Time');
xlabel('Time (yr)'); ylabel('Velocity');
lineWidthStart = startValue;
for k = 1:numToPlot
    plot(all_tSol{k}, all_lambda_vr{k}, [colorList{k},'-'], ...
         'LineWidth', lineWidthStart, 'DisplayName', labels{k});
    lineWidthStart = lineWidthStart - decrement;
end
legend('Location','best');
% lambda_{v_\theta}(t)
title('\lambda_{v} vs. Time');
xlabel('Time (yr)'); ylabel('\lambda_{v}');
lineWidthStart = startValue;
for k = 1:numToPlot
    plot(all_tSol{k}, all_lambda_vtheta{k}, [colorList{k},'-'], ...
         'LineWidth', lineWidthStart, 'DisplayName', labels{k});
    lineWidthStart = lineWidthStart - decrement;
end
legend('Location','best');
...
% Figure 5: Control Input Evolution
figure('Color','white','Position',[1200 0 1000 1000]);
% Subplot 1: Control components u1 and u2 vs. Time
 hold on; grid on;
title('Control Components vs. Time');
xlabel('Time (yr)'); ylabel('Control Input');
lineWidthStart = startValue;
for k = 1:numToPlot
    plot(all_tSol{k}, all_u1{k}, [colorList{k},'--'], ...
         'LineWidth', lineWidthStart, 'DisplayName', [labels{k},' u_1']);
    plot(all_tSol{k}, all_u2{k}, [colorList{k},':'], ...
         'LineWidth', lineWidthStart, 'DisplayName', [labels{k},' u_2']);
    lineWidthStart = lineWidthStart - decrement;
end
lineWidthStart = startValue;
for k = 1:numToPlot
    plot(all_tSol{k}, all_uMag{k}, [colorList{k},'-'], ...
         'LineWidth', lineWidthStart, 'DisplayName', [labels{k},' |u|']);
    lineWidthStart = lineWidthStart - decrement;
end
legend('Location','best');
set(gca, 'FontSize', 20);
...
% ------------------------------------------------------------------------
    ...
    % Optimal control (from ∂H/∂u = 0 => u = -0.5 * lambda_v)
    u1 = -0.5 * lvr;
    u2 = -0.5 * lvth;
    ...
    %
    % Hamiltonian H = u^2 + lambda_r * f_r + lambda_θ * f_θ + lambda_{v_r} * f_{v_r} + lambda_{v_θ} * f_{v_θ}
    % with u^2 = (u1^2 + u2^2).
    %
    ...
    % dH/dθ = 0 (no explicit dependence)
    dHdtheta = 0;
    dlth_dt  = -dHdtheta;
