% AAE590 Final Project - Terrain Elevation Particle Filter
% Derek Yu
...
cfg.dem.point_step   = 1;  % for the optional point cloud visualization
...
cfg.viz.save_video  = false;  % Save particle video
cfg.viz.video_file  = 'particles.mp4'; % Video name
...
% Precompute square patch measurements for LiDAR truth
...
truth.psi = zeros(cfg.N,1); % heading [rad]
...
meas_patch = nan(cfg.N, patch_count);    
truth_patch_mean = nan(cfg.N,1);    
for k = 1:cfg.N
...
% Storage for particle snapshots to viz evolution
...
% Initial estimate from the prior particle set (avoids huge initial error)
est.x(1)   = sum(particles.x .* particles.w);
est.y(1)   = sum(particles.y .* particles.w);
est.psi(1) = angle_mean(particles.psi, particles.w);
dx0 = particles.x - est.x(1);
dy0 = particles.y - est.y(1);
est.sigma_pos(1) = sqrt(sum(particles.w .* dx0.^2) + sum(particles.w .* dy0.^2));
...
for k = 2:cfg.N
    ...
    % Propagate each particle with control + process noise
    % The importance proposal p(x_k | x_{k-1}, u_k)
    ...
    % Predict square-footprint measurement for each particle from DEM
    ...
    % Compute importance weights using Gaussian likelihood over patch
    ...
    if all(isnan(z_patch))
        ...
        num_pts = sum(valid, 2);
        diag.num_valid_pts(k) = mean(num_pts); % Average over particles
        % Use mean squared error per patch so likelihood doesn't vanish solely
        % because the patch has many samples
    ...
    % Normalize weights + guard against degeneracy
    ...
    % Compute effective sample size and resample if needed
    ...
    % Store particle snapshot for visualization
