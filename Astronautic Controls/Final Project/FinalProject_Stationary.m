...
alpha = 0.0;                   % fuel consumption coeff [s/km]
...
function [X,U,constraints] = createDecisionVarsAndConstraints(...
    ...
    % boundary conditions
    constraints = [constraints, X{end}(1:3)==0];
...
function plotResults(X, U, N, dt, t0, tF, T_max)
    ...
    throttleLevel = (sigmaVals / 0.25) * 100;   % % corrected
    ...
    title('Control Inputs and Magnitude','FontSize',20); set(gca,'FontSize',20);
end